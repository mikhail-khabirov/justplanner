...
export function startAnnualOffer(): void {
    ...
    safeLocalStorage.removeItem(OFFER_DISMISSED_KEY); // clear dismissed flag for new registration
    const expiry = Date.now() + 24 * 60 * 60 * 1000; // +24h
    safeLocalStorage.setItem(OFFER_EXPIRY_KEY, expiry.toString());
}

/** Get remaining ms until offer expires. Returns 0 if expired or not started. */
export function getOfferRemainingMs(): number {
    const raw = safeLocalStorage.getItem(OFFER_EXPIRY_KEY);
    if (!raw) return 0;
...
export function dismissOffer(): void {
    safeLocalStorage.setItem(OFFER_DISMISSED_KEY, '1');
}

/** Check if offer was permanently dismissed */
export function isOfferDismissed(): boolean {
    return safeLocalStorage.getItem(OFFER_DISMISSED_KEY) === '1';
}
