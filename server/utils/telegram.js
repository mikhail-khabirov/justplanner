    ...
    try {
        const result = await pool.query(`
            SELECT 
                ...
                COUNT(*) FILTER (WHERE p.created_at >= (CURRENT_DATE AT TIME ZONE 'Europe/Moscow') AND p.status = 'succeeded' AND ((p.description LIKE '%автопродление%' OR p.description LIKE '%после триала%') AND p.description NOT ILIKE '%годов%')) AS renew_today,
                COALESCE(SUM(p.amount) FILTER (WHERE p.created_at >= (CURRENT_DATE AT TIME ZONE 'Europe/Moscow') AND p.status = 'succeeded' AND ((p.description LIKE '%автопродление%' OR p.description LIKE '%после триала%') AND p.description NOT ILIKE '%годов%')), 0) AS renew_sum_today,
                COUNT(*) FILTER (WHERE p.created_at >= NOW() - INTERVAL '24 hours' AND p.status = 'succeeded' AND ((p.description LIKE '%автопродление%' OR p.description LIKE '%после триала%') AND p.description NOT ILIKE '%годов%')) AS renew_24h,
                COALESCE(SUM(p.amount) FILTER (WHERE p.created_at >= NOW() - INTERVAL '24 hours' AND p.status = 'succeeded' AND ((p.description LIKE '%автопродление%' OR p.description LIKE '%после триала%') AND p.description NOT ILIKE '%годов%')), 0) AS renew_sum_24h,
                COUNT(*) FILTER (WHERE p.created_at >= NOW() - INTERVAL '7 days' AND p.status = 'succeeded' AND ((p.description LIKE '%автопродление%' OR p.description LIKE '%после триала%') AND p.description NOT ILIKE '%годов%')) AS renew_week,
                COALESCE(SUM(p.amount) FILTER (WHERE p.created_at >= NOW() - INTERVAL '7 days' AND p.status = 'succeeded' AND ((p.description LIKE '%автопродление%' OR p.description LIKE '%после триала%') AND p.description NOT ILIKE '%годов%')), 0) AS renew_sum_week,
